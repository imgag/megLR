...
def get_reference_for_transcriptome_alignment(wildcards):
    """
    Get the correct FASTA reference for transcript alignment:

    - For per-sample approach, use the FASTA files in individual sample
      folder.
    - For aggregate approach return either the novel stringtie merged
      reference or the regular cDNA reference.
    """
    if wildcards['method'] == "reference":
        return config['ref']['cDNA']
    if wildcards['method'] == "stringtie-consensus":
        return "isoform_counts/all_samples_stringtie.isoforms.fasta"
    else:
        return f"Sample_{wildcards['sample']}/{wildcards['sample']}.{wildcards['method']}.fasta"
