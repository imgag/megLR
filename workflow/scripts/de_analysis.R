...
cts <- as.matrix(read.csv("de_analysis/all_counts.tsv", sep="\t", row.names="Reference", stringsAsFactors=FALSE))
...
trs_cts <- counts(d)
write.table(trs_cts, file="de_analysis/all_counts_filtered.tsv",sep="\t")
...
write.table(gene_cts, file="de_analysis/all_gene_counts.tsv",sep="\t")
