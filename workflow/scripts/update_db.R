...
folder <- "/mnt/share/data/ont_run_qc/runs"
out <- "test"

# Regex to match folder names
subf <- na.omit(str_match(list.dirs(folder, recursive = F), regex("(?:Q\\w{9}_)?\\d{5}\\w*_\\d{5}$")))
...
# Read report header
  ...
  # Read pycoQC json
    qc_data <- rjson::fromJSON(file = paste0(folder, "/", f, "/", f, ".pycoQC.json")) %>%
...
# Export data to CSV
write_csv(dt, file = paste0(out, ".csv"))
...
openxlsx::saveWorkbook(wb, paste0(out, ".xlsx"), overwrite = TRUE)


### PLotting
source("workflow/scripts/theme_cas.R")
...
p_weeks <- ggplot(dttw, aes(x = week, fill = project)) +
  ...
  mytheme
ggsave("weeks.png", height = 5, width = 8, plot = p_weeks)
...
ggsave("len_yield.png", height = 5, width = 8, plot = p_len_yield)
...
p_position <- 
  dttw %>% 
    ...
    mytheme 
ggsave("position_yield.png", height = 5, width = 8, plot = p_position)
